...
type AreaBody = { name?: unknown; description?: unknown; areaType?: unknown; isActive?: unknown };
...
export class ServiceAreasController {
  ...
  async list(@Headers('x-chatpro-inbox-key') key='', @Query('company') slug='') {
    ...
    const {data,error}=await this.supabase.getClient().from('service_areas')
      .select('id,name,description,area_type,is_active,is_default,created_at').eq('company_id',company.id).order('created_at');
  ...
  async create(@Headers('x-chatpro-inbox-key') key='', @Query('company') slug='', @Body() body:AreaBody={}) {
    ...
    const areaType=this.areaType(body.areaType); const {data,error}=await this.supabase.getClient().from('service_areas').insert({company_id:company.id,name,description,area_type:areaType,is_active:true,is_default:false}).select('id,name,description,area_type,is_active,is_default,created_at').single();
  ...
  async update(@Headers('x-chatpro-inbox-key') key='', @Query('company') slug='', @Param('areaId') areaId='', @Body() body:AreaBody={}) {
    ...
    const changes:any={}; if(body.name!==undefined) changes.name=this.name(body.name); if(body.description!==undefined) changes.description=this.desc(body.description); if(body.areaType!==undefined) changes.area_type=this.areaType(body.areaType);
    ...
    const {data,error}=await this.supabase.getClient().from('service_areas').update(changes).eq('id',existing.id).eq('company_id',company.id).select('id,name,description,area_type,is_active,is_default,created_at').single();
  ...
  private areaType(v:unknown){if(v===undefined||v===null||v==='')return null;if(v!=='sales'&&v!=='service')throw new BadRequestException('Tipo de área no válido.');return v;}
  private out(x:any){return{id:x.id,name:x.name,description:x.description??'',areaType:x.area_type==='sales'||x.area_type==='service'?x.area_type:null,isActive:x.is_active!==false,isDefault:x.is_default===true,createdAt:x.created_at??null};}
}

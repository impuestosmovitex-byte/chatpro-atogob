...
async function run(request: NextRequest, method: 'GET' | 'PUT') {
  ...
  if (method === 'PUT' && !canManage(session)) {
    return NextResponse.json(
      { ok: false, error: 'No tienes permiso para administrar horarios.' },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    const target = new URL(`${apiBase}/support-settings`);
